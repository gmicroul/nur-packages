...
let
  ...
  gdbm3 = gdbm.overrideAttrs (_old: rec {
    version = "1.8.3";
...
stdenv.mkDerivation rec {
  ...
  src =
    let
      urlBase = "https://softwareupdate.vmware.com/cds/vmw-desktop/ws/${version}/${build}/linux/";
      file = "VMware-Workstation-${version}-${build}.x86_64.bundle";
    in
    "${
      fetchzip {
  ...
  unpackPhase =
    ...
    in
    ''
      ${vmware-unpack-env}/bin/vmware-unpack-env -c "sh ${src} ${vmware-tools} --extract unpacked"
